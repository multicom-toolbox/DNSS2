...
for i in range(len(sequence_file)):
    ...
    test_labels = featuredata[:,0:8]#(169, 3)
...
if chkfiles(acc_history_out):
    ...
    with open(acc_history_out, "w") as myfile:
        myfile.write("time\t netname\t dataset\t filtnum\t layernum\t kernelsize\t Accuracy\t Loss\t batchsize\n")
if (batchsize > 0):
    acc_history_content = "%s\t %s\t %s\t %i\t %i\t %s\t %s\t %s\t %i\n" % (time_str, model_prefix, test_list_temp[len(test_list_temp)-1],nb_filters,nb_layers,filtsize,test_acc,test_loss, batchsize)
else:
    acc_history_content = "%s\t %s\t %s\t %i\t %i\t %s\t %s\t %s\n" % (time_str, model_prefix, test_list_temp[len(test_list_temp)-1],nb_filters,nb_layers,filtsize,test_acc,test_loss)
