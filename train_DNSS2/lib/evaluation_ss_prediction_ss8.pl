...
use DN_SSpred2 qw(newDN timeprint sort_scores score_dnss check_err all_dnss_files score_server_prediction_ss8); 
use Getopt::Long;
...
my ($AvgQ8, $AvgSov) = score_server_prediction_ss8($predir, \@testlist, $thistag,$ssa, $server, $eva_ext,$seq_ext);
next if check_err($AvgQ8, $logfile);
