...
@ISA = qw(Exporter);
@EXPORT = ();
@EXPORT_OK = qw(generate_feature_for_convolution generate_feature_for_convolution_DNSS2_exp generate_feature_for_convolution_DNSS2_exp_8class  generate_testfeature_for_convolution_DNSS2 newDN make_features  make_features_8class all_testfeatures make_testfeatures print_features
	train_DN test_DN all_dnss_files predict_SS  predict_SS2 predict_SS2_8c make_dnss_file make_dnss_file_8c
...
sub generate_feature_for_convolution_DNSS2_exp_8class { 
	my ($logfile, $outdir, $trlist_ref, $telist_ref, $featdirs, $featopts) = @_;
	...
	foreach (@trainlist){
		...
		my $ssafile = $ssadir . "$_.8class.ssa";
		my $pssmfile = $pssmdir . "$_.pssm";
		my $pssm_uniref50file = $pssm_uniref50dir . "$_.pssm";
		my $hmmEmfile = $hmmEmdir . "$_.hmm2prob";
		my $hmmTrfile = $hmmTrdir . "$_.hmm2prob";
		...
		my @features = make_features_8class(\@files, $featopts);
		unless (check_err($features[0], $logfile)){
       $TFfile = $outdir . "$_.fea";
       open (OUT, ">$TFfile") or return "err: newDN: Couldn't open file $TFfile\n";
      	foreach (@features){
...
sub make_features_8class {
	push (@_, 0);
	return features_8class(@_);
}
...
sub features_8class { 
	...
	@{ $SSHotCode{"G"} }  = qw/0 0 0 0 0 0 0 1/;
	@{ $SSHotCode{"H"} }  = qw/0 0 0 0 0 0 1 0/;
	@{ $SSHotCode{"I"} }  = qw/0 0 0 0 0 1 0 0/;
	@{ $SSHotCode{"T"} }  = qw/0 0 0 0 1 0 0 0/;
	@{ $SSHotCode{"E"} }  = qw/0 0 0 1 0 0 0 0/;
	@{ $SSHotCode{"B"} }  = qw/0 0 1 0 0 0 0 0/;
	@{ $SSHotCode{"S"} }  = qw/0 1 0 0 0 0 0 0/;
	@{ $SSHotCode{"C"} }  = qw/1 0 0 0 0 0 0 0/;
	@{ $SSHotCode{"-"} }  = qw/0 0 0 0 0 0 0 0/; #only used if boosting
  ...
  if($pssm_uniref50)
  {
	...
	# Collect HMM information
	#################################
  #perl /storage/htc/bdm/Collaboration/jh7x3/DeepCov_SS_SA_project/DNSS2.0/lib/testhmm.pl /storage/htc/bdm/Collaboration/Zhiye/SSP/DNSS2/datasets/feature_construct/uniclust30_2017_10_hmm/1UI0-A.hmm2prob
  #print "Loading $hmmEmfile\n";
  open (HMM, $hmmEmfile) or return ("err: features: Couldn't open hmm file $hmmEmfile");  
  my @hmmlines = <HMM>;
  close HMM;
...
sub predict_SS2_8c {
	...
	my $probfile = $outdir . "$prot.ss8.prob";
	my $predfile = $outdir . "$prot.ss8.pred";

	unless (-f $featfile){
		return ("err: predict_SS: File $featfile not found");
	} 
	`python2 $GLOBAL_PATH/scripts/test_dnss_single_8class.py $model $weight $featfile $predfile $probfile &> $outdir/$tag.log`;
	return ($probfile, $predfile);
}
...
sub make_dnss_file_8c {
	...
	# If multiple prob files are used, their sum is calculated
	# Then this result is used to convert probabilities to structure
	my @probs = sum_probs_8c(@predfiles);
	my @ss = probs_to_ss_8c(@probs);
	my @best = best_probs(@probs);
